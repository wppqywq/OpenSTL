...
from .config import GEOM_CONFIG, IMG_SIZE, SEQUENCE_LENGTH, TRAIN_SIZE, VAL_SIZE, TEST_SIZE, HEATMAP_SIGMA, RANDOM_SEED
...
# Configuration Parameters - using constants from config.py
LOCAL_CONFIG = {
    'img_size': IMG_SIZE,
    ...
    for b in range(B):
        for t in range(T):
            if mask[b, t]:
                ...
                # Calculate Gaussian heatmap (no normalization by default)
...
class UnifiedGeometricDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if repr_type == 'sparse':
            ...
            # Create Gaussian heatmap frames (no normalization here)
